create database IPL_STADIUM;
drop database ipl_stadium;
...
select bidder_id , bid_status , count(bid_status) as Wins_of_each_bidder, 
count(BID_STATUS) * 100.0 / sum(count(BID_STATUS)) over() as Percentage_of_wins_of_each_bidder
#count(*) * 100.0 / sum(count(*)) over()
from ipl_bidding_details 
where bid_status = 'won'
group by bidder_id
order by Percentage_of_wins_of_each_bidder desc ;

#2.	Display the number of matches conducted at each stadium with stadium name, city from the database
select * from ipl_match_schedule;
select * from ipl_stadium;

select s.stadium_name , s.city  , ms.stadium_id , count(ms.stadium_id) as Number_of_matches
...
select t.team_name , t.team_id , m.toss_winner,
 m.match_winner , count(m.match_winner) as match_wins,
count(m.match_winner) * 100 / sum(count(m.match_winner)) over() as percentage_of_wins
from ipl_match m join ipl_team t 
...
select count(bd.bidder_id) as Total_bids , t.team_name , t.remarks
...
select count(bd.bidder_id) as Total_bids , t.team_name , t.remarks
from ipl_team t join ipl_bidding_details bd
on t.team_id =  bd.bid_team 
...
select t.team_name , t.remarks , ts.matches_won , ts.tournmt_id , t.team_id ,m.match_winner , m.win_details 
from ipl_team t join  ipl_team_standings ts
on t.team_id = ts.team_id
join ipl_match m 
on t.team_id = m.team_id1 
group by ts.tournmt_id , t.team_name;

select t.team_name , t.remarks , ts.matches_won , ts.tournmt_id , t.team_id 
from ipl_team t join  ipl_team_standings ts
on t.team_id = ts.team_id;

select t.team_id , t.team_name , t.remarks , m.match_winner , m.win_details 
from ipl_team t join ipl_match m 
on t.team_id = m.team_id1;

select sum(matches_won) from ipl_team_standings;

#6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select * from ipl_team;
select * from ipl_team_standings;

select t.team_name , t.remarks , ts.tournmt_id , ts.matches_played , ts.matches_won , ts.matches_lost 
from ipl_team t join ipl_team_standings ts
on t.team_id = ts.team_id;
...
select p.player_id , p.player_name , tp.player_role , tp.remarks 
from ipl_team_players tp join ipl_player p
on  p.player_id = tp.player_id
where tp.remarks = 'TEAM - MI' and tp.player_role = 'Bowler';
...
select t.team_name , t.remarks , count(t.team_id) as number_of_all_rounders , tp.player_role 
from ipl_team_players tp join ipl_team t 
on tp.team_id = t.team_id 
where tp.player_role like 'All-Rounder'
group by t.team_name
having number_of_all_rounders > 4
order by number_of_all_rounders desc;
